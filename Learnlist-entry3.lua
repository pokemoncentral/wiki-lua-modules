...
| style="background:#FFFFFF; border:1px solid #D8D8D8;" | ${p1}${games}]=],
		entry(p[11] or '', p[2] or 'Stramontante', lib.makeNotes(p[10] or ''),
...
z.tutor = function(frame)
    local p = lib.sanitize(mw.clone(frame.args))
    return table.concat{lib.tutorgames{{'RF', p[11]}, {'VF', p[11]},
...
z.tutornull = function(frame)
    return [[|-
! style="background:#FFFFFF; border:1px solid #D8D8D8;" colspan="12" | Questo Pokémon non impara nessuna mossa dall'Esperto Mosse.]]
end
