...
--]]
styles.classesStyles = function(pdfs, classes, stys)
    pdfs = type(pdfs) == 'string' and mw.text.split(pdfs, ' ') or pdfs
    pdfs = table.map(pdfs or {}, function(pdf)
        return pdf:find('^box-') and pdf or 'box-' .. pdf
    end)

    classes = table.merge(pdfs, parser.parseClasses(classes or {}))
