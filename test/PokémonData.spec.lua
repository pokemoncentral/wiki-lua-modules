...
-- Tests 1, 6
-- Standard case
table.insert(tests, { pokeData.getName({ args = { "0398" } }), "Staraptor" })

-- Few digits
table.insert(tests, { pokeData.getName({ args = { "65" } }), "Alakazam" })

-- getName alt form
table.insert(tests, { pokeData.getName({ args = { "0681S" } }), "Aegislash" })
...
table.insert(
    tests,
    { pokeData.getFormName({ args = { "800A" } }), "Necrozma Ali dell'Aurora" }
)
...
table.insert(
    tests,
    { pokeData.getFormName({ args = { "774R" } }), "Nucleo Rosso" }
)

-- Base form
table.insert(
    tests,
    { pokeData.getFormName({ args = { "422" } }), "Mare Ovest" }
)
...
table.insert(
    tests,
    { pokeData.getAbil2({ args = { "Alakazam" } }), "Forza Interiore" }
)
...
table.insert(
    tests,
    { pokeData.getAbil1({ args = { "94", gen = "5" } }), "Levitazione" }
)
...
table.insert(
    tests,
    { pokeData.getType2({ args = { "082", gen = "1" } }), "Elettro" }
)

-- Gradient types
table.insert(
    tests,
    { pokeData.gradTypes({ args = { "400" } }), "normale-acqua" }
)
table.insert(
    tests,
    { pokeData.gradTypes({ args = { "035", gen = "1" } }), "normale-normale" }
)
...
table.insert(
    tests,
    { pokeData.getStat({ args = { "189", "spdef", gen = "2" } }), 85 }
)
...
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { " Luxray " } }), "black-text" }
)
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { " Gengar " } }), "white-text" }
)
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { " 065" } }), "black-text" }
)
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { "249" } }), "white-text" }
)

-- Alt forms
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { " alakazamM " } }), "black-text" }
)
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { "487O" } }), "white-text" }
)

-- Useless forms (probably they shouldn't even work, but here we go)
table.insert(
    tests,
    { pokeData.getPokeTextColor({ args = { " shellosE " } }), "white-text" }
)
